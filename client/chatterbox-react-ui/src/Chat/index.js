...
export default class Chat extends Component {
  constructor(props) {
    ...
    this.state.receiverId = `chatUser${Math.floor(Math.random() * 100 + 1)}`;
  ...
  render() {
    ...
    return (
      <div className="card chat-app">
        ...
        <div className="chat">
          <TextBx senderId={this.state.senderId} receiverId={this.state.receiverId} token={this.state.token} />
        </div>
      </div>
    );
  }
...
const useWs = (senderId, receiverId, token) => {
  ...
  const socket = new WebSocket(`ws://localhost:8080/webs/chat/${senderId}/${receiverId}/${token}`);
...
const TextBx = ({ senderId, receiverId, token }) => {
